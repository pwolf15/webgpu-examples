...
impl State {
  ...
  fn update(&mut self, dt: std::time::Duration) {
    ...
    let model_mat1 = transforms::create_transforms([-2.0,-1.5,-0.5],
    ...
    let model_mat2 = transforms::create_transforms([-0.5,1.0,-1.5],
      [0.0, dt.sin(), dt.cos()], [1.0,1.0,1.0]);
    let mvp_mat2 = self.project_mat * self.view_mat * model_mat2;
    let mvp_ref2:&[f32; 16] = mvp_mat2.as_ref();
    self.init.queue.write_buffer(&self.uniform_buffer, self.uniform_offset, bytemuck::cast_slice(mvp_ref2));
  }
...
fn main() {
  ...
  window.set_title(&*format!("{}", "ch12-two-cubes"));
